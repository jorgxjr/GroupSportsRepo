...
namespace GroupSports.DL.DALI
{
    public class TeamRepository : ITeamRepository
    {
        ...
        {
            return context.team.Where(t => t.available).ToList();
        }

        public team findById(int id)
        {
            return context.team.FirstOrDefault(x => x.id == id && x.available);
        }

        public List<team> findByName(string name)
        {
            return context.team.Where(x => x.teamName == name && x.available).ToList();
        }
